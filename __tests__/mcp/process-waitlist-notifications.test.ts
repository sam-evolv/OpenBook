// Heavy tests for the shared waitlist-notification processor.
// Cron auth + delegation tests live in __tests__/cron/notify-waitlist.test.ts.
...
describe('processWaitlistNotifications', () => {
  it('returns zeros when the queue is empty', async () => {
    const out = await processWaitlistNotifications({ limit: 50 });
    expect(out).toEqual({ processed: 0, sent: 0, failed: 0, skipped: 0 });
  ...
  it('happy path: creates hold, sends email, marks sent + waitlist notified', async () => {
    seedPending(1);
    const out = await processWaitlistNotifications({ limit: 50 });
    expect(out).toMatchObject({ processed: 1, sent: 1 });
  ...
  it('SLOT_UNAVAILABLE: marks notification skipped, no email sent', async () => {
    ...
    const out = await processWaitlistNotifications({ limit: 50 });
    expect(out).toMatchObject({ processed: 1, skipped: 1 });
    expect(resendSendMock).not.toHaveBeenCalled();
  ...
  it('skips when waitlist has no email (defensive — handler enforces email at intake)', async () => {
    seedPending(1);
    waitlistRow = {
      data: { ...(waitlistRow.data as Record<string, unknown>), contact_email: null },
      error: null,
    };
    const out = await processWaitlistNotifications({ limit: 50 });
  ...
  it('respects the limit argument (lazy path requests 5; cron requests 50)', async () => {
    seedPending(20);
    const lazy = await processWaitlistNotifications({ limit: 5 });
    expect(lazy.processed).toBe(5);

    // Reset queue + counters for the second batch.
    seedPending(60);
    rpcMock.mockClear();
    resendSendMock.mockClear();
    updateCalls.length = 0;
    const big = await processWaitlistNotifications({ limit: 50 });
    expect(big.processed).toBe(50);
  });
});
