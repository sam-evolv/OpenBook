...
- **Preview Auth Fix PR — infrastructure cleanup.** Every review-gate PR so far has re-hit the same preview-access friction. Resolve in a dedicated infra PR after Flash Sales Stage 2 lands: (1) disable Vercel Deployment Protection SSO on preview deployments — Supabase magic-link auth is sufficient gating and the SSO layer blocks clean preview testing; (2) fix the post-login redirect target — signing in currently lands on bare `/dashboard` which 404s, should redirect to `/dashboard/overview`; (3) document the owners-row seeding step for new auth users (Studio insert or trigger on `auth.users` insert), since a missing row silently breaks `requireCurrentBusiness`. Not urgent but recurring friction — the 2026-04-23 preview-access hour on PR #37 was the third time this has cost real time.
